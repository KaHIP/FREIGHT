...
int parse_parameters(int argn, char **argv, 
        ...
        struct arg_str *filename                             = arg_strn(NULL, NULL, "FILE", 1, 1, "Path to hypergraph file to partition.");
#if defined MODE_CONVERTFORMAT
        struct arg_str *filename_output                      = arg_str0(NULL, "output_filename", NULL, "Specify the name of the output hypergraph (in pin-set-list format) (standard: <name of input hypergraph in hmetis format>.pinsl");
#else
        ...
        struct arg_int *k                                    = arg_int1(NULL, "k", NULL, "Number of blocks to partition the hypergraph.");
        ...
        struct arg_str *one_pass_algorithm		     = arg_str0(NULL, "one_pass_algorithm", NULL, "One-pass partitioning algorithm (hashing|freight). (Default: freight)");
	...
	// translation of graphs
        ...
        struct arg_lit *two_phase_partition_format	     = arg_lit0(NULL, "two_phase_partition_format", "Input partition is given in the format offered by the two-phase edge-partitioning algorithm. (Default: disabled)");
...
#elif defined MODE_CONVERTFORMAT
                filename_output,  
	...
	if(suppress_file_output->count > 0) {
...
#ifdef MODE_NETLIST
                if (strcmp("hashing", one_pass_algorithm->sval[0]) == 0) {    
                        partition_config.one_pass_algorithm = ONEPASS_HASHING;         
                } else if (strcmp("freight", one_pass_algorithm->sval[0]) == 0) {    
		...
		if (strcmp("hmetis_pinsetlist", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = HMETIS_TO_PINSETLIST;         
                } else if (strcmp("hmetis_netlist", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = HMETIS_TO_NETLIST;         
                } else if (strcmp("hmetis_edgelist", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = HMETIS_TO_EDGELIST;         
                } else if (strcmp("hmetis_matrixmarket", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = HMETIS_TO_MATRIXMARKET_NODE_TO_NET;         
                } else if (strcmp("metis_pinsetlist_edgetonet", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = METIS_TO_PINSETLIST_EDGE_TO_NET;         
                } else if (strcmp("metis_netlist_edgetonet", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = METIS_TO_NETLIST_EDGE_TO_NET;         
                } else if (strcmp("metis_pinsetlist_nodetonet", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = METIS_TO_PINSETLIST_NODE_TO_NET;         
                } else if (strcmp("metis_netlist_nodetonet", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = METIS_TO_NETLIST_NODE_TO_NET;         
                } else if (strcmp("metis_hmetis_edgetonet", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = METIS_TO_HMETIS_EDGE_TO_NET;         
                } else if (strcmp("metis_hmetis_nodetonet", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = METIS_TO_HMETIS_NODE_TO_NET;         
                ...
                } else if (strcmp("hmetis_print_pin_count", graph_translation_specs->sval[0]) == 0) {    
                        partition_config.graph_translation_specs = HMETIS_PRINT_PIN_COUNT;         
                } 
        }
		...
		partition_config.valid_neighboring_nets.resize(partition_config.parallel_nodes);
	}
        ...
        if(stream_sampling_type->count > 0) {
			...
			std::cout << "Parameter \"" <<  stream_sampling_type->sval[0] << "\" entered in --stream_sampling_type does not work for pinsetlist input format." << std::endl;
			exit(0);
#endif
	...
	if(sampling_threashold->count > 0) {
        ...
        if(two_phase_partition_format->count > 0) {
                partition_config.two_phase_partition_format = true;
        }
