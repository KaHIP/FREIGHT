...
public:
        ...
        void setPartitionConfig( PartitionConfig & config ) {
                ...
                config.compact_bin_id = new std::vector<unsigned long long int> (config.k,0);
		...
		if (this->config != NULL && this->config->compact_bin_id != NULL) {
			delete this->config->compact_bin_id;
		}
	};
