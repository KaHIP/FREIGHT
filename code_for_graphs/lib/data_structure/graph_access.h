...
class graph_access {
        ...
        public:
                graph_access() { m_max_degree_computed = false; m_max_degree = 0; graphref = new basicGraph(); m_separator_block_ID = 2; using_ghost_nodes = false;}
		...
		NodeWeight getImplicitGhostNodes(NodeID node);
		void setImplicitGhostNodes(NodeID node, NodeWeight id);

                //to be called if combine in meta heuristic is used
                void resizeSecondPartitionIndex(unsigned no_nodes);
		void resizeImplicitGhostNodesList(unsigned no_nodes);
                ...
                void incrementEdgeWeight(EdgeID edge, EdgeWeight weight);
		...
		bool has_compressed_ghost_nodes();
        ...
        private:
                ...
                std::vector<NodeWeight> m_ghost_nodes;
		bool	     using_ghost_nodes;
};
...
inline void graph_access::stream_finish_construction() {
        graphref->stream_finish_construction();
}
...
inline bool graph_access::has_compressed_ghost_nodes() {
	return using_ghost_nodes;
}

inline void graph_access::resizeImplicitGhostNodesList(unsigned no_nodes) {
        m_ghost_nodes.resize(no_nodes);
	using_ghost_nodes = true;
}
...
inline NodeWeight graph_access::getImplicitGhostNodes(NodeID node) {
#ifdef NDEBUG
        return m_ghost_nodes[node];
#else
        return m_ghost_nodes.at(node);
#endif
}

inline void graph_access::setImplicitGhostNodes(NodeID node, NodeWeight id) {
#ifdef NDEBUG
        m_ghost_nodes[node] = id;
#else
        m_ghost_nodes.at(node) = id;
#endif
}
...
inline void graph_access::incrementEdgeWeight(EdgeID edge, EdgeWeight weight){
#ifdef NDEBUG
        graphref->m_edges[edge].weight += weight;        
#else
        graphref->m_edges.at(edge).weight += weight;        
